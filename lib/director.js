...
const { tryLocalFirst } = require("./aiProviders");
...
async function askClaude(system, userMsg, maxTokens = 600) {
  try {
    return await tryLocalFirst({ system, prompt: userMsg, maxTokens, timeoutMs: 20000 }, async () => {
      if (!process.env.ANTHROPIC_API_KEY) throw new Error("Anthropic not configured");
      const res = await axios.post(
        "https://api.anthropic.com/v1/messages",
        { model: "claude-sonnet-4-6", max_tokens: maxTokens, system, messages: [{ role: "user", content: userMsg }] },
        { headers: { "Content-Type": "application/json", "x-api-key": process.env.ANTHROPIC_API_KEY, "anthropic-version": "2023-06-01" }, timeout: 20000 }
      );
      return res.data?.content?.[0]?.text || null;
    });
  } catch (e) {
    console.warn("[director] AI error:", e.message);
    return null;
  }
}
