...
// Send offer letter email via Gmail API
async function sendOfferEmail(gmail, { toEmail, applicantName, orgName, offerLetter }) {
  if (!toEmail) return;

  const firstName = applicantName.split(" ")[0];
  const subject   = `Offer Letter — ${orgName}`;
  const raw = [
    `To: ${toEmail}`,
    `Subject: ${subject}`,
    "MIME-Version: 1.0",
...
module.exports = { makeOAuthClient, getOrgClients, findFreeSlot, createInterviewEvent, sendInterviewEmail, sendOfferEmail, SCOPES };
