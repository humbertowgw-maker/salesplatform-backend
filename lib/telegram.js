...
async function sendDirectorBriefing({ content, date, repCount }) {
  const preview = (content || "").slice(0, 400);
  await sendMessage(
    `📋 <b>DIRECTOR BRIEFING — ${date}</b>\n\n${preview}${content?.length > 400 ? "…" : ""}\n\n<i>${repCount} rep${repCount !== 1 ? "s" : ""} assessed</i>`
  );
}

module.exports = { sendMessage, sendAppointmentBooked, sendFollowupSummary, sendAlert, sendDirectorBriefing, isConfigured };
