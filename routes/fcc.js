  ...
  try {
    const aiRes = await axios.post(
      "https://api.anthropic.com/v1/messages",
      {
        ...
        max_tokens: 900,
        system: `You are a broadband market expert with comprehensive knowledge of ISP coverage across all 50 US states. Given a city, state, and ZIP code, list ALL ISPs that realistically serve that area. Return JSON only: {"providers":[{"brand_name":"string","technology_description":"string","technology_code":number,"max_advertised_download_speed":number,"max_advertised_upload_speed":number,"low_latency":boolean}]}. Tech codes: 50=Fiber,40=Cable HFC,71=Licensed Fixed Wireless,62=Starlink,60=Satellite GSO,10=DSL. Include AT&T Fiber in major metros, Quantum Fiber in Denver/SLC/Phoenix/Portland, Ziply Fiber in WA/OR/ID/MT only, Xfinity/Comcast nationwide, Spectrum nationwide, T-Mobile and Verizon Home Internet nationwide, Starlink nationwide.`,
    ...
    if (providers.length > 0) {
      ...
      const intelRes = await axios.post("https://api.anthropic.com/v1/messages",
        { model: "claude-sonnet-4-6", max_tokens: 500, system: "AT&T B2B sales strategist. Return JSON only: { summary, talkingPoints: [3 strings], primaryCompetitor, angle, competitiveScore }", messages: [{ role: "user", content: `Address: ${fullAddress}\nProviders:\n${providerList}\nGive AT&T sales intel.` }] },
